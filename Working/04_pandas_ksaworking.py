...
## rename uses a param coloumns, then add a dictionary that shows 'Old_name':'New_name'
drinks.rename(columns={'beer_servings':'beer', 'wine_servings':'wine'})
## doing drinks = drinks.rename() is not efficient on big datasets
## so we use the argument inplace=True
drinks.rename(columns={'beer_servings':'beer', 'wine_servings':'wine'}, inplace=True)

# replace all column names
## create list, then assign the columns to the .columns attribute
drink_cols = ['country', 'beer', 'spirit', 'wine', 'liters', 'continent']
drinks.columns = drink_cols

# replace all column names when reading the file
## you can use this arg when you're reading it in with .read_csv()
## header=0 means header is on line 0, header=None means NO Header
drinks = pd.read_csv('drinks.csv', header=0, names=drink_cols)

# add a new column as a function of existing columns
## Kind of like excel, instead of FOR LOOP, it adds everything in the same
## row and assigns it to the string in the list
drinks['servings'] = drinks.beer + drinks.spirit + drinks.wine
drinks['mL'] = drinks.liters * 1000
drinks['servings_daily'] = drinks.servings / 365

## 2 ways to access series, drinks.servings or drinks["servings"]
...
## dropna=false inludes missing values
 
drinks.continent.value_counts()              # excludes missing values
drinks.continent.value_counts(dropna=False)  # includes missing values

# find missing values in a Series
## generates a boolean seriers... just like drinks.beer > 5
...
## sum(axis=0) sums each column, this needs to be memorized... not totally intutive
## sum(axis=1) sums each row
...
## !!! drinks.isnull.sum() is great because it gives us a count of missing
## values by column, helping us understand how many NULLs we have
drinks.isnull()             # DataFrame of booleans
drinks.isnull().sum()       # count the missing values in each column

# drop missing values
## need to use inplace=true to make it stick
## .fillna() fills all drniks 
...
ufo.describe()
[ufo[column].value_counts().head(1) for column in ufo.columns]
...
ufo[(ufo.State == 'ME') & (ufo.City == 'Ashland')]
...
newUfoCols = ['City','Colors_Reported','Shape_Reported','State','Time']
ufo.columns = newUfoCols 
ufo.columns
## Panda has a ton of string methods including str.replace
...
## split .groupby('item') --> apply to series then function
## dataframe.split().series.apply() --> automatically combines
## dataframe.groupby().series.function()
drinks.groupby('continent').beer.mean()

# for each continent, count the number of occurrences
## value_counts is a built in split apply combine!
drinks.continent.value_counts()
...
## .agg --> pass a list of functions, returns in a nice table
